...
static void J_scope_gimbal_test(void)
{
    ...
    //YAW陀螺仪最终角度 是个定值
    final_yaw_angle_set=(int32_t)(filtered_horizontal_pixel_jscope-(delayed_yaw_absolute_angle*RAD_TO_DEGREE)); //减去delay数ms后的编码器的值，在filter.h中定义
    //PITCH编码器最终角度 是个定值
...
static void GIMBAL_Control_loop(Gimbal_Control_t *gimbal_control_loop)
{
    ...
    {
        //gyro角度控制
        gimbal_motor_drive_mode_control_yaw(&gimbal_control_loop->gimbal_yaw_motor);//驾驶随式-底盘跟随云台控制
    }
...
////        gimbal_motor_absolute_angle_control(&gimbal_control_loop->gimbal_pitch_motor);
			  //gyro角度控制
        gimbal_motor_drive_mode_control_pitch(&gimbal_control_loop->gimbal_pitch_motor);//驾驶模式-底盘跟随云台控制
    }
...
//		fp32 pitch_limit_offset=DEGREE_TO_RAD*0;//减小降低下界，增大提高下界
    ...
    else if(gimbal_motor->relative_angle_set<-pitch_limit*3.0f)//上界，60°
    {
...
//		fp32 pitch_limit_offset=DEGREE_TO_RAD*0;//减小降低下界，增大提高下界
    if(gimbal_motor->relative_angle_set>pitch_limit*1.5f)//下界
    {
