...
static void gimbal_motor_aim_control_gyro_yaw(Gimbal_Motor_t *gimbal_motor)
{
    if (gimbal_motor == NULL)
    {
		...
		{
			tx2.horizontal_pixel=tx2.raw_horizontal_pixel+13;//赋值自瞄数据 //+13激光对中心
...
/*******************************************************************/
    if(abs((int)delta_yaw)>15){
			if(delta_yaw>15){//If left to right, need faster?
				gimbal_motor->motor_gyro_set = GIMBAL_PID_Calc(&gimbal_motor->gimbal_motor_absolute_angle_pid, gimbal_motor->absolute_angle, final_absolute_yaw_angle_set, gimbal_motor->motor_gyro*20);
			}
...
//		gimbal_motor->relative_angle_set+=delta_pitch;
		...
		{
			tx2.vertical_pixel=tx2.raw_vertical_pixel-17;//赋值自瞄数据
			//改变最终绝对角度
			final_relative_pitch_angle_set=gimbal_motor->relative_angle_set+delta_pitch;
		}
...
static void gimbal_motor_relative_angle_control_yaw(Gimbal_Motor_t *gimbal_motor)//普通模式下YAW电机
{
    if (gimbal_motor == NULL)
    {
		...
		//校准摄像头虚拟深度时用
		tx2.horizontal_pixel=tx2.raw_horizontal_pixel+13;//赋值自瞄数据
