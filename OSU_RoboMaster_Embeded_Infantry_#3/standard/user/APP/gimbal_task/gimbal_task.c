...
static void gimbal_motor_drive_mode_control_yaw(Gimbal_Motor_t *gimbal_motor);
static void gimbal_motor_drive_mode_control_pitch(Gimbal_Motor_t *gimbal_motor);
...
static void GIMBAL_relative_angle_limit(Gimbal_Motor_t *gimbal_motor, fp32 add);//仅校准用
static void GIMBAL_absolute_angle_limit(Gimbal_Motor_t *gimbal_motor, fp32 add);//仅校准用
...
static void GIMBAL_Control_loop(Gimbal_Control_t *gimbal_control_loop)
{
    ...
    {
        //gyro角度控制
        gimbal_motor_drive_mode_control_yaw(&gimbal_control_loop->gimbal_yaw_motor);//跟随模式-云台控制
    }
    ...
    {
        //enconde角度控制
        gimbal_motor_drive_mode_control_pitch(&gimbal_control_loop->gimbal_pitch_motor);
//				//gyro角度控制
...
//驾驶模式下无法自瞄，仅为驾驶车辆用
static void gimbal_motor_drive_mode_control_yaw(Gimbal_Motor_t *gimbal_motor)//驾驶模式下YAW电机
{
    if (gimbal_motor == NULL)
    {
        return;
...
//驾驶模式下无法自瞄，仅为驾驶车辆用
static void gimbal_motor_drive_mode_control_pitch(Gimbal_Motor_t *gimbal_motor)//驾驶模式下PITCH电机
{
    if (gimbal_motor == NULL)
    {
        return;
    ...
    if (tx2.raw_vertical_pixel!=9999 && tx2.raw_vertical_pixel!=0)//如果原始自瞄数据不返回跟丢或未发送
    {
		...
		//限制云台pitch电机上下幅度
    fp32 pitch_limit=DEGREE_TO_RAD*200;//减小降低上界，增大提高上界
//		fp32 pitch_limit_offset=DEGREE_TO_RAD*0;//减小降低下界，增大提高下界
    if(gimbal_motor->relative_angle_set>pitch_limit)//下界，20°
    {
        gimbal_motor->relative_angle_set=pitch_limit;
        buzzer_on(60,5000);
    }
    else if(gimbal_motor->relative_angle_set<-pitch_limit)//上界，60°
    {
        gimbal_motor->relative_angle_set=-pitch_limit;
        buzzer_on(60,5000);
    }
...
//    static fp32 final_absolute_yaw_angle_set; //yaw电机最终相对目标角度
    ...
    delta_yaw=(fp32)((filtered_final_angle_set[0]-(PREDICTION_TIME+prediction_time_offset)*RAD_TO_DEGREE*filtered_aim_data[2])//预测
                     *-DEGREE_TO_RAD*1.0f);
...
//		if(gimbal_motor->final_absolute_angle_set>yaw_limit)//左界
//		{
//			gimbal_motor->final_absolute_angle_set=yaw_limit;
//			buzzer_on(60,5000);
//		}
//		else if(gimbal_motor->final_absolute_angle_set<-yaw_limit)//右界
//		{
//			gimbal_motor->final_absolute_angle_set=-yaw_limit;
//			buzzer_on(60,5000);
//		}
...
//		fp32 pitch_limit_offset=DEGREE_TO_RAD*0;//减小降低下界，增大提高下界
    if(gimbal_motor->relative_angle_set>pitch_limit)//下界
    {
        gimbal_motor->relative_angle_set=pitch_limit;
        buzzer_on(60,5000);
    }
